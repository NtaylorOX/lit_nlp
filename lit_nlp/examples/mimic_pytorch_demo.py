...
# make dataset class
class MimicDataset(lit_dataset.Dataset):
  """Loader for MultiNLI development set."""

  LABELS = ['AcuteMedicine',
                'Cardiology',
                'Gastroenterology',
                'Neurology',
                'Obstetrics',
...
class MimicModel(lit_model.Model):
  ...
  def predict_minibatch(self, inputs):
    ...
    for output in tqdm(utils.unbatch_preds(detached_outputs)):
...
def main(_):
    ...
    cache_dir = "/mnt/sdg/niallt/hf_models/"
    data_dir = "/mnt/sdg/niallt/mimic3-icd9-data/intermediary-data/triage/valid.csv"
    ...
    models = {"mimic": MimicModel(model_path, cache_dir=cache_dir)}
    # Load SST-2 validation set from TFDS.
    datasets = {"mimic_dev": MimicDataset(path = data_dir)}
